...
if len (s.argv) != 2: # Checks if the number of command-line arguments is not equal to 2 (the script name and one additional argument). 
    ...
    s.exit()
response = rq.get(f"https://itunes.apple.com/search?entity=song&limit=1&term={s.argv[1]}")  #Acts as a client, sending a request to the iTunes API to search for a song 
...
#Example of using json.dumps() to print the entire response in a readable format:
